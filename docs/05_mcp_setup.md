...
このテンプレートは4つの MCP サーバーをデフォルトで設定しています。
...
- `search_papers` — 論文検索（実装コードとベンチマーク付き）
- `list_paper_results` — 論文のベンチマーク結果
- `list_paper_repositories` — 論文に紐づく GitHub リポジトリ
- `list_paper_tasks` — タスク別の論文一覧
- `search_research_areas` — 研究領域の検索
...
- `Node.js` がインストール済みであること（paperswithcode）
